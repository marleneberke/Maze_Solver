...
#################################################################################
#returns the number of moves that you can see ahead until an intersection where you don't know what to do
function count_certain_moves_ahead(current_location::Coordinate, current_node_matrix::Matrix{TreeNode})
    m = 0
    current_node = current_node_matrix[current_location.x, current_location.y]
    while length(current_node.children)==1 #while there's only one way to go
        m = m + 1
        current_location = current_node.children[1]
        current_node = current_node_matrix[current_location.x, current_location.y]
    end
    return m
end
...
function conduct_search(current_location::Coordinate, best_location::Coordinate, best_val::Float64, counter::Int64, depth_limit::Int64, current_node_matrix::Matrix{TreeNode})
    ...
    while !isempty(locations_to_visit) && (best_val > 0)
        ...
        if to_search.depth < depth_limit# && to_search.location != goal_location #if you're at the goal, don't add its children
            shuffled = homebrew_shuffle(to_search_node.children)
